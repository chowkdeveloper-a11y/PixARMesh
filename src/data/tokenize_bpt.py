import trimesh
import numpy as np
from .utils import quantize_points, dequantize_points
...
def tokenize_layout(quantized_pts, block_size=8, offset_size=16):
    block_id = quantized_pts // offset_size
    block_id = (
        block_id[:, 0] * block_size**2 + block_id[:, 1] * block_size + block_id[:, 2]
    )
    offset_id = quantized_pts % offset_size
    offset_id = (
    ...
    sequence = block_coords
    return sequence.flatten()


def detokenize_layout(sequence, block_size=8, offset_size=16):
...
def get_block_representation(
    ...
    coords = quantize_points(coords, num_pos_tokens=offset_size * block_size)
...
def decode_block(sequence, compressed=True, block_size=8, offset_size=16):
    num_pos_tokens = block_size * offset_size
    ...
    coords = dequantize_points(coords, num_pos_tokens=num_pos_tokens)
...
def BPT_deserialize(
    ...
    for i in range(len(sequence)):
        sub_seq = []
        if not use_special_block and (
            sequence[i] == special_token or i == len(sequence) - 1
        ):
            ...
            special_block_base <= sequence[i] < special_block_base + block_size**3
            or i == len(sequence) - 1
        ):
            if i != 0:
                sub_seq = (
                ...
                if (
                    special_block_base
                    <= sub_seq[0]
                    < special_block_base + block_size**3
                ):
...
if __name__ == "__main__":
    ...
    import torch
    import trimesh
    from .utils import normalize_vertices, quantize_mesh, dequantize_points
    ...
    vertices, faces = quantize_mesh(vertices, faces, num_pos_tokens=128)
    vertices = dequantize_points(vertices, num_pos_tokens=128)

    cleaned_mesh = to_mesh(vertices, faces)
    codes = BPT_serialize(cleaned_mesh)
    coordinates = BPT_deserialize(codes)
    faces = torch.arange(1, len(coordinates) + 1).view(-1, 3)

    out_mesh = to_mesh(coordinates, faces, post_process=True)
    out_mesh.export("reconstructed.ply")
